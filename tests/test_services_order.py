...
class CreateOrderTestCase(TestCase):
    ...
    def setUp(self):
        self.user = _create_user('create_user')
        self.goods = _create_goods()
        self.sku = _create_sku(self.goods, stock=10, price=99.00)
        cache.clear()

    @patch('baykeshop.contrib.shop.services.order_service.reverse')
    def test_create_single_sku_order(self, mock_reverse):
        """单SKU正常下单"""
        mock_reverse.return_value = '/pay/1/'
        from baykeshop.contrib.shop.services.order_service import OrderService
        from baykeshop.contrib.shop.models.orders import BaykeShopOrdersGoods

        goods_data = [{'sku': self.sku, 'quantity': 2}]
        order = OrderService.create_order(
            user=self.user, goods_data=goods_data,
            source='', receiver='张三', phone='13800138000',
            address='北京', email=''
        )

        self.assertEqual(order.user, self.user)
        self.assertEqual(order.status, 0)  # UNPAID
        self.assertEqual(float(order.pay_price), 198.00)  # 99 * 2

        order_goods = BaykeShopOrdersGoods.objects.filter(orders=order)
        self.assertEqual(order_goods.count(), 1)
        self.assertEqual(order_goods.first().quantity, 2)
    ...
    @patch('baykeshop.contrib.shop.services.order_service.reverse')
    def test_create_multi_sku_order(self, mock_reverse):
        """多SKU订单"""
        mock_reverse.return_value = '/pay/1/'
        from baykeshop.contrib.shop.services.order_service import OrderService
        from baykeshop.contrib.shop.models.orders import BaykeShopOrdersGoods

        sku2 = _create_sku(self.goods, stock=5, price=50.00, sku_sn='SKU-002')
        goods_data = [
            {'sku': self.sku, 'quantity': 2},
            {'sku': sku2, 'quantity': 1},
        ]

        order = OrderService.create_order(
            user=self.user, goods_data=goods_data,
            source='', receiver='李四', phone='13900139000',
            address='上海', email=''
        )

        self.assertEqual(float(order.pay_price), 248.00)  # 198 + 50
        self.assertEqual(BaykeShopOrdersGoods.objects.filter(orders=order).count(), 2)

        self.sku.refresh_from_db()
        sku2.refresh_from_db()
        self.assertEqual(self.sku.stock, 8)
        self.assertEqual(sku2.stock, 4)

    @patch('baykeshop.contrib.shop.services.order_service.reverse')
    def test_create_order_from_carts_clears_cart(self, mock_reverse):
        """购物车来源下单后清理购物车"""
        mock_reverse.return_value = '/pay/1/'
        from baykeshop.contrib.shop.services.order_service import OrderService
        from baykeshop.contrib.shop.models.goods import BaykeShopCarts

        cart = BaykeShopCarts.objects.create(user=self.user, sku=self.sku, quantity=2)
        goods_data = [{'sku': self.sku, 'quantity': 2}]

        OrderService.create_order(
            user=self.user, goods_data=goods_data,
            source='carts', receiver='王五', phone='13700137000',
            address='广州', email=''
        )
    ...
    @patch('baykeshop.contrib.shop.services.order_service.reverse')
    def test_create_order_insufficient_stock_rollback(self, mock_reverse):
        """库存不足时 DB CHECK 约束触发事务回滚"""
        from baykeshop.contrib.shop.services.order_service import OrderService
        from baykeshop.contrib.shop.models.orders import BaykeShopOrders, BaykeShopOrdersGoods
        from django.db import IntegrityError

        mock_reverse.return_value = '/pay/1/'
        goods_data = [{'sku': self.sku, 'quantity': 20}]  # 库存仅10

        with self.assertRaises(IntegrityError):
            OrderService.create_order(
                user=self.user, goods_data=goods_data,
                source='', receiver='赵六', phone='13600136000',
                address='深圳', email=''
            )

        # 事务回滚 → 无订单 / 无订单商品
        self.assertEqual(BaykeShopOrders.objects.filter(user=self.user).count(), 0)
        self.assertEqual(BaykeShopOrdersGoods.objects.count(), 0)
        # 库存不变
        self.sku.refresh_from_db()

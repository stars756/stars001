...
# ============================================================
# 缓存常量 — templatetags 中的缓存配置
# ============================================================
_TT_CACHE_TIMEOUT = {
    'navs': 300,          # 导航分类：5分钟
    'carts_count': 60,   # 购物车数量：1分钟
}
...
def sort_template(request, filter_params: str, sort_field="created_time", **kwargs):
    ...
    # Django ORM: "-"前缀 = 降序(DESC)，无前缀 = 升序(ASC)
    is_desc = bool(sort and sort.startswith("-"))
    is_asc = bool(sort and not sort.startswith("-"))
    if sort_field == sort:
        sort_field = f"-{sort}"
...
def sku_template(spu):
    ...
    for sku in sku_queryset:
        ...
        skus_data[key] = {
            ...
            "sales": sku.sales,
            "is_virtual":sku.goods.is_virtual
        }
...
def carts_count(user):
    ...
    if not user.is_authenticated:
        return 0
    cache_key = f"tt:carts:{user.id}"
    cached = cache.get(cache_key)
    if cached is not None:
        return cached
    count = BaykeShopCarts.objects.filter(user=user).count()
    cache.set(cache_key, count, timeout=_TT_CACHE_TIMEOUT['carts_count'])
    return count
...
def paydata(skus, has_carts=False):
    ...
    for sku in skus:
        item = {}
        real_sku = sku.sku if has_carts else sku
        item["sku"] = real_sku.id
        item["price"] = str(real_sku.price)
        item["quantity"] = sku.quantity
        item["is_virtual"] = real_sku.goods.is_virtual
        results.append(item)
    return results
...
def navs(is_nav=True):
    """导航分类（带5分钟缓存，避免每个页面都查库）"""
    cache_key = f"tt:navs:{int(is_nav)}"
    cached = cache.get(cache_key)
    if cached is not None:
        return cached
